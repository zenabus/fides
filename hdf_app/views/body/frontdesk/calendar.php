          ...
          <div class="mr-3">
            <div class="form-check form-check-inline mb-0">
              <label class="form-check-label">
                <input class="form-check-input filter-shift" type="checkbox" value="AM" checked>
                AM
            ...
            <div class="form-check form-check-inline mb-0">
              <label class="form-check-label">
                <input class="form-check-input filter-shift" type="checkbox" value="PM" checked>
                PM
                <span class="form-check-sign"></span>
              </label>
            </div>
                  ...
                  <tr>
                    ...
                    <?php for ($i = 1; $i <= $days; $i++) { ?>
                      ...
                      <?php if ($data) { ?>
                        ...
                        $res_type = strtolower($data['reservation_type'] ?? '');
                        $icon = '';
                        if ($res_type == 'online') {
                          $icon = '<i class="fa fa-globe mr-1" title="Online"></i> ';
                        } elseif (in_array($res_type, ['arrival/tentative', 'confirmed'])) {
                          $icon = '<i class="fa-solid fa-person-walking mr-1" title="Walk-In"></i> ';
                        }

                        $ampm = '';
                        $checkin_time = null;
                        if ($data['booking_type'] == 'Check In') {
                          $checkin_time = $data['booking_added'];
                        } elseif (in_array($data['reservation_status'], [0, -1])) {
                          $checkin_time = !empty($data['actual_checkin_time']) ? $data['actual_checkin_time'] : $data['booking_updated'];
                        }
...
<script>
  ...
  $('.filter-shift').change(function() {
    const showAM = $('input.filter-shift[value="AM"]').is(':checked');
    const showPM = $('input.filter-shift[value="PM"]').is(':checked');

    $('.with-data').each(function() {
      const isAM = $(this).hasClass('shift-AM');
      const isPM = $(this).hasClass('shift-PM');

      if (isAM || isPM) {
        if ((isAM && showAM) || (isPM && showPM)) {
